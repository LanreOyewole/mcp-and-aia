...
async def execute_a2a_agent(agent_card_url: str,
    ...
    async with httpx.AsyncClient(timeout=30) as httpx_client:
        ...
        mcp_agent_card_url = agent_card_url + "/agent_card"
        print(f"Debug-01: ... {mcp_agent_card_url}")
        # 1. Fetch the agent card JSON
        resp = await httpx_client.get(mcp_agent_card_url)
        print(f"Debug-02: ... {resp}")
        ...
        policy_skill = AgentSkill(
            ...
            description="Answers queries about HR policies, including payroll, benefits, and workplace conduct.",
            tags=["HR", "policies", "payroll", "policy"],
            examples=[
        ...
        # 2. Parse into AgentCard
        #agent_card = AgentCard.model_validate(agent_card_json)
        print(f"Debug-04: ... ")

        # 3. Instantiate A2AClient correctly
        client = A2AClient(httpx_client, policy_agent_card)
        print(f"Debug-05: ... {client}")

        '''# FIX: Use the factory method to load the agent card properly
        client = await A2AClient.get_client_from_agent_card_url(
            httpx_client,
            agent_card_url
        )'''

        print("Agent URL received :", client)

        input_dict = {"user": user, "surname": surname, "prompt": prompt}
        ...
        text = response_json["result"]["parts"][0]["text"]
...
class RouterHRAgent:
    ...
    def __init__(self, model, system_prompt, user, surname, debug=True):
        ...
        router_graph.add_node("Payroll_Agent", self.policy_agent_node)
