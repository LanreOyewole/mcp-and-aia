...
class TimeOffDatastore:
    ...
    def __init__(self, db_path=":memory:"):
        ...
        self.conn = self.get_connection(db_path=db_path)
        print("Creating tables and seeding data")
        self.create_tables()
        self.seed_data()

    #Get a fresh connection to the database
    def get_connection(self, db_path=":memory:"):
        print("Getting connection to TimeOffDatastore")
    ...
    def seed_data(self):
        ...
        employees = [
            ("Alice", "Veli", "Turkish", "Female", "Istanbul", 20, 5),
            ("Bob", "Smith", "American", "Male", "New York", 15, 3),
            ("Charlie", "Brown", "British", "Male", "London", 25, 10),
            ("Allison", "Krause", "Canadian", "Female", "Toronto", 18, 2),
            ("Diana", "Prince", "Greek", "Female", "Athens", 30, 15),
            ("Ethan", "Hunt", "Australian", "Male", "Sydney", 22, 7)
        ]
        for name, surname, nationality, gender, place_of_birth, allowed, consumed in employees:
            cursor.execute('''
                INSERT OR IGNORE INTO employee (name, surname, nationality, gender, place_of_birth, allowed_days, consumed_days)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', (name, surname, nationality, gender, place_of_birth, allowed, consumed))
        self.conn.commit()
        cursor.close()

    # Get details of a specific employee given their name and surname
    def get_employee_details(self, name, surname):
        cursor = self.conn.cursor()
        cursor.execute('''
    ...
    def get_timeoff_balance(self, employee_name):
        #conn = self.get_connection(db_path=":memory:")
        cursor = self.conn.cursor()
        cursor.execute('''
...
if __name__ == "__main__":
    ...
    ds.add_timeoff_request("Alice", "2026-06-10", 2)
    print("Alice's balance after request:", 
            ds.get_timeoff_balance("Alice"))
    print("Alice's details:", ds.get_employee_details("Alice", "Veli"))
